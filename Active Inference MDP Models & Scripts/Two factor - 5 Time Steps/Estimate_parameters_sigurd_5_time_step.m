function [DCM] = Estimate_parameters_sigurd_5_time_step(DCM)
...
if isfield(M.pE,'la')&&isfield(M.pE,'rs')
    mdp.C{1}(1,2:T) = 0; %Null
    mdp.C{1}(2,2:T) = -mdp.la/2; %loss
    mdp.C{1}(3,2:T) = mdp.rs; %Win

elseif isfield(M.pE,'la')
    mdp.C{1}(1,2:T) = 0; %Null
    mdp.C{1}(2,2:T) = -mdp.la/2; %loss
    mdp.C{1}(3,2:T) = rs; %Win


elseif isfield(M.pE,'rs')
    mdp.C{1}(1,2:T) = 0; %Null
    mdp.C{1}(2,2:T) = -la/2; %loss
    mdp.C{1}(3,2:T) = mdp.rs; %Win
...
[MDP.u]    = deal(Y{j}); % Add actions to each trial
...
for i = 1:numel(Y) % Get probability of true actions for each trial (loop all trials)
    for j = 1:numel(Y{i}(2,:)) % Only get probability of the second (controllable) state factor
        
        L = L + log(MDP(i).P(:,Y{i}(2,j),j) + eps); % sum the (log) probabilities of each (EXCECUTED, hence the :,j, j) action
