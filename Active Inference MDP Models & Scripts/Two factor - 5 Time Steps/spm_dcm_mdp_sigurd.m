function [DCM] = spm_dcm_mdp_sigurd(DCM)
...
L     = 0;
for i = 1:numel(Y); % All trials
    for j = 1:numel(Y{1}(2,:)); %Only loop through controlled actions. 
        L = L + log( MDP(i).P(:,Y{i}(2,j),j)s+ eps); %Find log p(x) of each controlled action.
    end
end
