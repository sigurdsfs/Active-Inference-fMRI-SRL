function Q = spm_MDP_VB_game_tutorial_sigurd(MDP)
...
for i = 1:Nt
    ...
    if any(MDP(i).s(2,:) ~= 1)
        s(:,i) = MDP(i).s(:, MDP(i).s(2,:) ~= 1);
    else
        s(:,i) = MDP(i).s(:,end); %take last state
    end 
    
    o(:,i) = MDP(i).o(1,:)'; %Observation of null/loss/win 
    
    if any(MDP(i).u(2,:) ~= 1)
        act_prob(:,i) = MDP(i).P(:,:,MDP(i).u(2,:) ~= 1)';
        act(:,i) = MDP(i).u(2,MDP(i).u(2,:) ~= 1);
    else 
        act_prob(:,i) = MDP(i).P(:,2:3,end)'; %Take last action 
        act(:,i) = MDP(i).u(2,end); %Take last action 
    end
...
yticklabels({'','Do Nothing','Choose Green','','Choose Yellow'})
% Performance
%--------------------------------------------------------------------------
...
for i = 1:size(o,2)
    ...
    if any(MDP(i).o(1,:) ~= 1)
        j(i,1) = MDP(i).o(1, MDP(i).o(1,:) ~= 1 ) -1; % Did we win or not. 
    else
        j(i,1) = MDP(i).o(1,end) -1;
    end 
...
for f = 1
    ...
    subplot(5*Nf, 1, Nf*4 + f), image(64*(1 - d{f})), hold on
    ...
    plot((j),col{2},'linestyle','none','marker','.','MarkerSize',MarkerSize); 
...
%     end
    ...
    yticklabels({'Green-Better','Yellow-Better'})
